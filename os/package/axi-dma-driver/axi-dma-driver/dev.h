...
struct axi_dma_channel
{
  ...
  void               *mem;            // adresse virtuelle du début de la zone mémoire des buffers cohérents
  dma_addr_t         *handles;        // adresses physiques des buffers
  void               *sg_mem;         // adresse virtuelle du segment de mémoire pour les descripteurs scatter-gather
  ...
  dma_addr_t          completed;      // adresse physique de la zone de buffers disponible pour l'utilisateur
  u32                 completed_id;   // id du premier buffer disponible pour l'utilisateur
};

struct axi_dma_dev
{
  struct resource        *io_resource;    // ressources physiques
