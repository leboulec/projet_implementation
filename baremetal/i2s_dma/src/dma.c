...
int run_fft(fft_config_t* fft_params, int* stim_buf, int* result_buf)
{
	...
	status = dma_xfer(result_buf, stim_buf);
	if (status != XST_SUCCESS)
	{
...
int dma_xfer(int* s2mm_buf, int* mm2s_buf)
{
	// ***** Local variables *****
	int status = 0;
	...
	// Flush cache
	Xil_DCacheFlushRange((u32)s2mm_buf, 4);
	...
	status = XAxiDma_SimpleTransfer(&axi_dma, (u32)s2mm_buf, 4, XAXIDMA_DEVICE_TO_DMA);
	if (status != XST_SUCCESS)
	{
	...
	status = XAxiDma_SimpleTransfer(&axi_dma, (u32)s2mm_buf, 4, XAXIDMA_DMA_TO_DEVICE);
	if (status != XST_SUCCESS)
	{
